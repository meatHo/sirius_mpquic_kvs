    ...
    if (event == picoquic_callback_stream_data && length > 0)
    {
        ...
        while (g_recvBuf.size() >= 5) // 최소 1 + 4 = 5바이트
        {
            ...
            if (type == PKT_TYPE_RTP)
            {
                ...
                // if (pktLen >= 12)
                // {
                //     uint8_t *rtp = g_recvBuf.data() + 5;
                //     uint8_t pt = rtp[1] & 0x7F;
                //     uint8_t nal_type = 0;
                //     if (pktLen >= 13)
                //         nal_type = rtp[12] & 0x1F; // NAL header
                //     static int dbg_cnt = 0;
                //     if (dbg_cnt++ < 20)
                //     {
                //         printf("[RTP] pt=%d, nal_type=%d (7=SPS,8=PPS,5=IDR,1=P)\n", pt, nal_type);
                //     }
                // }
...
GstElement *initGStreamerPipeline()
{
    std::string gst_pipeline =
        ...
        "rtpjitterbuffer latency=200 drop-on-latency=true ! " // 200→50ms
