...
static std::vector<uint8_t> g_recvBuf_s4;
static std::vector<uint8_t> g_recvBuf_s8;
...
#define SITEWISE_LAT_PROPERTY_ID  "6678b0ee-4c47-4c97-b083-9d19379ffabf"
#define SITEWISE_LON_PROPERTY_ID  "1d597a7d-7b75-4415-965a-ce36b38e0ce1"

// ── freeze 측정 ───────────────────────────────────────────────
static uint64_t g_last_rtp_rx_us = 0;
static std::mutex g_freeze_mtx;
static FILE* g_freeze_csv_fp = nullptr;
static const uint64_t FREEZE_THRESHOLD_US = 500000ULL;

// ── 시계 오차 허용 임계값 (NTP 동기화 후 잔여 오차 허용) ──────
static const uint64_t CLOCK_TOLERANCE_US = 50000ULL;  // 50ms
...
struct FrameArrival {
    uint64_t t_tx_us       = 0;
    uint64_t t_rx_uu_us    = 0;
    uint64_t t_rx_pc5_us   = 0;
...
static FILE* g_cl_csv_fp = nullptr;

struct EpochAdvisoryRecord {
    uint64_t t_adv_sent_us  = 0;
    uint64_t epoch_first_tx = 0;
};
static std::map<uint64_t, EpochAdvisoryRecord> g_adv_record;
...
static const uint64_t EPOCH_US    = 5000000ULL;
static const uint64_t DEADLINE_US = 200000ULL;
...
void sendGpsToSiteWise(const std::string& gpsJson) {
    std::string tagged = "GPS:" + gpsJson;
    {
        std::lock_guard<std::mutex> lk(g_swQueueMtx);
        if (g_swQueue.size() >= SW_QUEUE_MAX) g_swQueue.pop();
        g_swQueue.push(tagged);
    }
...
std::string makeSummaryJson(const std::string& jsonStr) {
    ...
    oss << "{\"f\":" << frameId << ",\"count\":" << objs.size() << ",\"top\":[";
...
void siteWiseWorkerThread() {
    ...
    while (keepRunning) {
        std::string jsonStr;
        {
            ...
            g_swQueueCv.wait(lk, [] { return !g_swQueue.empty() || !keepRunning; });
            if (!keepRunning && g_swQueue.empty()) break;
            jsonStr = std::move(g_swQueue.front());
            g_swQueue.pop();
        }
        // jsonStr 꺼낸 직후 맨 위에 추가
if (jsonStr.substr(0, 4) == "GPS:") {
    std::string gpsJson = jsonStr.substr(4);
    double lat = 0.0, lon = 0.0;
    size_t latPos = gpsJson.find("\"lat\":");
    size_t lonPos = gpsJson.find("\"lon\":");
    if (latPos != std::string::npos) lat = atof(gpsJson.c_str() + latPos + 6);
    if (lonPos != std::string::npos) lon = atof(gpsJson.c_str() + lonPos + 6);
    ...
    snprintf(cmd, sizeof(cmd),
        ...
        "--region %s "
        "--entries '[{\"entryId\":\"gps-lat-%ld\",\"assetId\":\"%s\","
        "\"propertyId\":\"%s\","
        "\"propertyValues\":[{\"value\":{\"doubleValue\":%.7f},"
        "\"timestamp\":{\"timeInSeconds\":%ld}}]},"
        "{\"entryId\":\"gps-lon-%ld\",\"assetId\":\"%s\","
        "\"propertyId\":\"%s\","
        "\"propertyValues\":[{\"value\":{\"doubleValue\":%.7f},"
        "\"timestamp\":{\"timeInSeconds\":%ld}}]}]' 2>&1",
        SITEWISE_REGION,
        nowSec, SITEWISE_ASSET_ID, SITEWISE_LAT_PROPERTY_ID, lat, nowSec,
        nowSec, SITEWISE_ASSET_ID, SITEWISE_LON_PROPERTY_ID, lon, nowSec);

    FILE* fp = popen(cmd, "r");
    if (fp) { char r[512]={0}; fread(r,1,sizeof(r)-1,fp); pclose(fp); }
    printf("[GPS->SW] lat=%.6f, lon=%.6f\n", lat, lon);
    continue;
}
        int64_t nowSec = (int64_t)std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::system_clock::now().time_since_epoch()).count();
        ...
        snprintf(cmd, sizeof(cmd),
            "aws iotsitewise batch-put-asset-property-value "
            "--region %s "
            "--entries '[{\"entryId\":\"yolo-%ld\",\"assetId\":\"%s\","
            "\"propertyId\":\"%s\",\"propertyValues\":[{"
            "\"value\":{\"stringValue\":\"%s\"},"
            "\"timestamp\":{\"timeInSeconds\":%ld}}]}]' 2>&1",
            SITEWISE_REGION, nowSec,
            SITEWISE_ASSET_ID, SITEWISE_PROPERTY_ID,
            escaped.c_str(), nowSec);
        FILE* fp = popen(cmd, "r");
        if (!fp) { printf("[SiteWise] popen 실패\n"); continue; }
        char result[1024] = {0};
        fread(result, 1, sizeof(result) - 1, fp);
        pclose(fp);
        if (strstr(result, "\"errorEntries\": []") || strstr(result, "\"errorEntries\":[]") || strlen(result) == 0)
...
// ── 시계 오차 허용 uplink 계산 헬퍼 ──────────────────────────
// rx_us: receiver 수신 시각, tx_us: sender 송신 시각
// 50ms 이내 오차면 허용, latency는 0으로 클램프
static inline bool uplink_valid(uint64_t rx_us, uint64_t tx_us) {
    if (rx_us == 0) return false;
    if (rx_us >= tx_us) return true;                    // 정상
    return (tx_us - rx_us) < CLOCK_TOLERANCE_US;        // 오차 허용
}

static inline uint64_t uplink_lat_us(uint64_t rx_us, uint64_t tx_us) {
    return (rx_us >= tx_us) ? (rx_us - tx_us) : (tx_us - rx_us);
}
    ...
    while (buf.size() >= 5) {
        ...
        if (type == PKT_TYPE_RTP) {
            if (!is_redundant_stream) {
                ...
                // ── freeze 측정 ───────────────────────────────
                struct timeval tv_rtp;
                gettimeofday(&tv_rtp, nullptr);
                uint64_t t_rtp_rx = (uint64_t)tv_rtp.tv_sec * 1000000ULL + tv_rtp.tv_usec;
                {
                    std::lock_guard<std::mutex> lk(g_freeze_mtx);
                    if (g_last_rtp_rx_us > 0) {
                        uint64_t gap_us = t_rtp_rx - g_last_rtp_rx_us;
                        if (gap_us > FREEZE_THRESHOLD_US) {
                            double freeze_duration_ms = gap_us / 1000.0;
                            printf("[Freeze] ★ freeze 감지! duration=%.1fms\n", freeze_duration_ms);
                            if (g_freeze_csv_fp) {
                                fprintf(g_freeze_csv_fp, "%llu,%.3f\n",
                                    (unsigned long long)g_last_rtp_rx_us, freeze_duration_ms);
                                fflush(g_freeze_csv_fp);
                            }
                        }
                    }
                    g_last_rtp_rx_us = t_rtp_rx;
                }
            }
            ...
            if (pktLen == sizeof(FrameMetaPacket)) {
...

                {
                    ...
                    if (t_rx > g_last_frame_rx_us) g_last_frame_rx_us = t_rx;
                }

                int path = g_incoming_path;

                {
                    ...
                    entry.lat          = fmeta.lat;
                    entry.lon          = fmeta.lon;
                    if (path == 0) entry.t_rx_uu_us  = t_rx;
                    else           entry.t_rx_pc5_us = t_rx;

                    // ── CSV 기록 (시계 오차 허용) ──────────────
                    if (g_csv_fp && entry.t_tx_us > 0) {
                        uint64_t rx_time = (path == 0) ? entry.t_rx_uu_us : entry.t_rx_pc5_us;
                        if (uplink_valid(rx_time, entry.t_tx_us)) {
                            uint64_t lat_us = uplink_lat_us(rx_time, entry.t_tx_us);
                            std::lock_guard<std::mutex> cl(g_csv_mtx);
                            fprintf(g_csv_fp, "%llu,%llu,%.3f,%d\n",
            ...
            gettimeofday(&tv_ack, nullptr);
            uint64_t t_ack_rx_us = (uint64_t)tv_ack.tv_sec * 1000000ULL + tv_ack.tv_usec;
            std::string ackJson((const char*)buf.data() + 5, pktLen);

            uint64_t epoch_ts  = 0, t_adv_rx_us = 0;
            ...
            uint64_t epoch_first_tx = 0, t_adv_sent_ec2 = 0;
            {
            ...
            t_adv_sent_ec2 = parse_u64("\"t_adv_sent_us\":");

            double adv_return_ms = (t_adv_sent_ec2 > 0 && t_adv_rx_us > t_adv_sent_ec2)
                ? (t_adv_rx_us - t_adv_sent_ec2) / 1000.0 : -1.0;
            ...
            int sent = g_adv_sent_count.load(), acked = g_ack_recv_count.load();
            ...
            EpochMetrics em;
            { std::lock_guard<std::mutex> lk(g_epoch_metrics_mtx); em = g_last_epoch_metrics; }
            ...
            if (g_all_csv_fp) {
                fprintf(g_all_csv_fp,
                    "%llu,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
                    (unsigned long long)epoch_ts,
                    em.uu_mean_ms, em.uu_p95_ms,
                    em.pc5_mean_ms, em.pc5_p95_ms,
                    em.twin_ms, em.adv_gen_ms, em.epoch_wait_ms,
                    adv_return_ms, adv_ret_mean, adv_ret_p95,
                    total_e2e_ms, e2e_mean, e2e_p95);
                fflush(g_all_csv_fp);
            }
            if (g_cl_csv_fp) {
                fprintf(g_cl_csv_fp, "%llu,%.3f,%.3f\n",
                    (unsigned long long)epoch_ts, adv_return_ms, total_e2e_ms);
                fflush(g_cl_csv_fp);
            }
        }
...
void dispatchQuicPacket(uint8_t *data, size_t len,
    ...
    if (peer_sin->sin_addr.s_addr == pc5_sin->sin_addr.s_addr &&
        ...
        p_req->length = len; p_req->v2xChannelId = g_channelID;
...
static std::atomic<int64_t> g_lastGpsSendSec(0);
...
GstElement *initGStreamerPipeline() {
    std::string gst_pipeline =
        ...
        "storage-size=128 fragment-duration=1000 framerate=30 ";
    GError *err = nullptr;
...
picoquic_quic_t *initQuic() {
    ...
    picoquic_quic_t *quic = picoquic_create(
        ...
        customQuicCallback, NULL, NULL, NULL, NULL,
    ...
    while (keepRunning) {
        ...
        if (recvLen > 0) {
            ...
            picoquic_incoming_packet(quic, rxBuf, recvLen,
                (struct sockaddr *)&from_addr, (struct sockaddr *)&uu_local,
                0, 0, picoquic_current_time());
...
void quicResponseThread(picoquic_quic_t *quic) {
    ...
    while (keepRunning) {
        ...
        if (sendLen > 0) dispatchQuicPacket(sendBuf, sendLen, &peer_addr, &local_addr);
...
void epochWorkerThread() {
    ...
    while (keepRunning) {
        ...
        uint64_t t_epoch_run_us = (uint64_t)tv_epoch.tv_sec * 1000000ULL + tv_epoch.tv_usec;
        ...
        int uu_total = 0, uu_met = 0, pc5_total = 0, pc5_met = 0;
        float speed_mps = 0.0f;
...

        {
            ...
            for (auto& [fid, e] : g_frame_map) {
                if (e.t_tx_us == 0 || e.t_tx_us < cutoff_us) continue;
                if (e.t_tx_us > last_tx) last_tx = e.t_tx_us;

                // ── Uu uplink (시계 오차 허용) ────────────────
                if (uplink_valid(e.t_rx_uu_us, e.t_tx_us)) {
                    uu_total++;
                    uint64_t uu_lat_us = uplink_lat_us(e.t_rx_uu_us, e.t_tx_us);
                    uu_lat_ms.push_back(uu_lat_us / 1000.0);
                    if (uu_lat_us <= DEADLINE_US) uu_met++;
                }
                // ── PC5 uplink (시계 오차 허용) ───────────────
                if (uplink_valid(e.t_rx_pc5_us, e.t_tx_us)) {
                    pc5_total++;
                    uint64_t pc5_lat_us = uplink_lat_us(e.t_rx_pc5_us, e.t_tx_us);
                    pc5_lat_ms.push_back(pc5_lat_us / 1000.0);
                    if (pc5_lat_us <= DEADLINE_US) pc5_met++;
                }
                speed_mps = e.speed_mps; lat = e.lat; lon = e.lon; sched_id = e.scheduler_id;
            }
            g_epoch_last_tx = last_tx;
            for (auto it = g_frame_map.begin(); it != g_frame_map.end(); )
                it = (it->second.t_tx_us > 0 && it->second.t_tx_us < cutoff_us)
                     ? g_frame_map.erase(it) : ++it;
        }

        auto calc_stats = [](std::vector<double>& v) -> std::pair<double,double> {
            if (v.empty()) return {0.0, 0.0};
            double mean = 0; for (auto x : v) mean += x; mean /= v.size();
            std::sort(v.begin(), v.end());
            return {mean, v[(size_t)(v.size() * 0.95)]};
        };
        ...
        float d_uu_ms = 0, d_pc5_ms = 0, uu_jit_ms = 0, pc5_jit_ms = 0;
        {
...

        {
            ...
            if (g_csv_fp) {
                fprintf(g_csv_fp,
                    ...
                    uu_dmf, pc5_dmf, uu_total, uu_met, pc5_total, pc5_met,
                    d_uu_ms, d_pc5_ms, uu_jit_ms, pc5_jit_ms,
        ...
        if (t_epoch_run_us - last_til_send_us < 1000000ULL) {
            printf("[Epoch] Uu=%d/%d(%.2f) PC5=%d/%d(%.2f) d_uu=%.1fms d_pc5=%.1fms\n",
                   uu_met, uu_total, uu_dmf, pc5_met, pc5_total, pc5_dmf, d_uu_ms, d_pc5_ms);
            continue;
        }
        ...
        if (g_til_sock >= 0) {
            ...
            int n = snprintf(json_buf, sizeof(json_buf),
                "{\"uu_deadline_met_frac\":%.4f,\"pc5_deadline_met_frac\":%.4f,"
                "\"p_uu\":%.4f,\"p_pc5\":%.4f,"
                "\"uu_jitter_ms\":%.3f,\"pc5_jitter_ms\":%.3f,"
                "\"d_uu_ms\":%.3f,\"d_pc5_ms\":%.3f,"
                "\"uu_completed_frames\":%d,\"pc5_completed_frames\":%d,"
                "\"scheduler_id\":%d,\"speed_mps\":%.3f,"
                "\"lat\":%.7f,\"lon\":%.7f,"
                "\"uu_mean_latency_ms\":%.3f,\"uu_p95_latency_ms\":%.3f,"
                "\"pc5_mean_latency_ms\":%.3f,\"pc5_p95_latency_ms\":%.3f,"
                "\"epoch_ts\":%llu}\n",
                uu_dmf, pc5_dmf, 1.0f-uu_dmf, 1.0f-pc5_dmf,
                uu_jit_ms, pc5_jit_ms, d_uu_ms, d_pc5_ms,
                uu_total, pc5_total, sched_id, speed_mps, lat, lon,
                uu_mean, uu_p95, pc5_mean, pc5_p95,
                (unsigned long long)now);

            if (send(g_til_sock, json_buf, n, MSG_NOSIGNAL) < 0) {
                printf("[Epoch] til_server 연결 끊김. 재연결 시도...\n");
                close(g_til_sock); g_til_sock = -1;
            } else {
                ...
                if (rlen > 0) {
                    ...
                    uint64_t t_adv_sent_us = (uint64_t)tv_send.tv_sec * 1000000ULL + tv_send.tv_usec;
                    {
                        std::lock_guard<std::mutex> lk(g_adv_mtx);
                        EpochAdvisoryRecord rec;
                        rec.t_adv_sent_us  = t_adv_sent_us;
                    ...
                    std::lock_guard<std::mutex> lk(quic_mutex);
                    if (g_server_cnx) {
                        picoquic_add_to_stream(g_server_cnx, ADVISORY_STREAM_ID, &type, 1, 0);
...
void tilConnectThread() {
    ...
    while (keepRunning) {
        if (g_til_sock >= 0) { std::this_thread::sleep_for(std::chrono::seconds(1)); continue; }
        int s = socket(AF_INET, SOCK_STREAM, 0);
        if (s < 0) { sleep(2); continue; }
        struct sockaddr_in addr{};
        addr.sin_family = AF_INET;
        addr.sin_port   = htons(7777);
        inet_pton(AF_INET, "15.164.95.212", &addr.sin_addr);
...
int main(int argc, char **argv) {
    ...
    // ── CSV 파일 열기 ──────────────────────────────────────────
    g_csv_fp = fopen("/home/loapp/sirius/receiver_SiteWise/til_latency.csv", "w");
    if (g_csv_fp) fprintf(g_csv_fp, "frame_id,t_rx_us,uplink_latency_ms,path\n");
    ...
    if (g_all_csv_fp)
        fprintf(g_all_csv_fp,
            ...
            "twin_update_ms,adv_gen_ms,epoch_wait_ms,"
            "adv_return_ms,adv_return_mean_ms,adv_return_p95_ms,"
            "total_e2e_ms,e2e_mean_ms,e2e_p95_ms\n");

    g_freeze_csv_fp = fopen("/home/loapp/sirius/receiver_SiteWise/til_freeze.csv", "w");
    if (g_freeze_csv_fp)
        fprintf(g_freeze_csv_fp, "freeze_start_us,freeze_duration_ms\n");
    ...
    std::thread sitewise_thread(siteWiseWorkerThread);
    std::thread epoch_thread(epochWorkerThread);
    std::thread til_conn_thread(tilConnectThread);
